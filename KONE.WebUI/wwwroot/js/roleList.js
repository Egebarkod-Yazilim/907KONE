$(document).ready(function () {
    var datatable = $('#rolelisttable').DataTable({
        "ajax": {
            ...
            },
            {
                ...
                "render": function (data, type, row, meta) {
                    return '<div class="hstack gap-2">' +
                        '<a aria-label="anchor" href="/Role/AddOrUpdateRole/' + row.id + '" class="btn btn-icon btn-wave waves-effect waves-light btn-sm btn-konfrut-light-4"><i class="ri-edit-line" data-bs-toggle="tooltip" data-bs-placement="top" title="Rol Detay"></i></a>' +
                        '</div>';
                }
            },
        ],
    ...
    $('#rolelisttable thead th').each(function () {
        var title = $(this).text();
